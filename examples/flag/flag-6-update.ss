...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname flag-6-update) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define-struct world (shapes              ;; list of the shapes on the flag
                      ...
                      blink-visible?      ;; is the current shape visible on blink?
...
;; Produces true if we can redo the world.
...
;; reposition-x-y: world number number -> world
;; Repositions the x,y coordinate in the misc-state of the world.
(define (reposition-x-y a-world x y)
...
(define (render-flag a-world)
  (draw-cross-cursor (misc-state-posn (world-misc-state a-world)) 
                     (render-shapes
                      (cond
                        [(world-blink-visible? a-world)
                         (cons (world-current-editing-shape a-world)
                               (world-shapes a-world))]
                        [else (world-shapes a-world)]))))


;; toggle-blinking: world -> world
;; Toggle the blink-visible? attribute of the world.
(define (toggle-blinking a-world)
  (update-world-blink-visible? a-world (not (world-blink-visible? a-world))))
